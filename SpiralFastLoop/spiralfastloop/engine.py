...
def _concatenate_batches(base: Any, extra: Any) -> Any:
    ...
    if isinstance(base, torch.Tensor):
        if not isinstance(extra, torch.Tensor):
            raise TypeError(
                "Extra inputs must mirror tensor structure of original batch."
            )
        return torch.cat([base, extra], dim=0)
    if isinstance(base, Mapping):
        if not isinstance(extra, Mapping):
            raise TypeError(
                "Trigger extra batch must be a mapping matching the original batch."
            )
        if set(base.keys()) != set(extra.keys()):
            raise KeyError(
                "Trigger extra batch keys must match the original batch keys."
            )
        merged = {k: _concatenate_batches(base[k], extra[k]) for k in base.keys()}
        if isinstance(base, MutableMapping):
            try:
                new_mapping: MutableMapping[Any, Any] = type(base)()
    ...
    if isinstance(base, list):
        if not isinstance(extra, (list, tuple)) or len(base) != len(extra):
            raise TypeError(
                "Extra batch must mirror the list structure of the base batch."
            )
        concatenated = [
            _concatenate_batches(base[idx], extra[idx]) for idx in range(len(base))
        ]
        return type(base)(concatenated)
    if isinstance(base, tuple):
        if not isinstance(extra, (list, tuple)) or len(base) != len(extra):
            raise TypeError(
                "Extra batch must mirror the tuple structure of the base batch."
            )
        concatenated = [
            _concatenate_batches(base[idx], extra[idx]) for idx in range(len(base))
        ]
...
def _infer_batch_size(batch: Any) -> int:
    ...
    if isinstance(batch, Mapping):
        ...
        if not candidate_values:
            raise ValueError("Unable to infer batch size from mapping inputs.")
        unique = set(candidate_values)
        if len(unique) != 1:
            raise ValueError("Inconsistent batch sizes detected in mapping inputs.")
    ...
    if isinstance(batch, (list, tuple)):
        ...
        if not candidate_values:
            if hasattr(batch, "__len__"):
                return len(batch)
            raise ValueError("Unable to infer batch size from sequence inputs.")
        unique = set(candidate_values)
        if len(unique) != 1:
            raise ValueError("Inconsistent batch sizes detected in sequence inputs.")
        return candidate_values[0]
    if batch is None:
...
class FastTrainer:
    ...
    def __init__(
        ...
        *,
        ...
        use_amp: Union[bool, str, None] = "auto",
        compile_mode: str = "reduce-overhead",
        grad_accum: int = 1,
        channels_last: bool = False,
        clip_grad_norm: Optional[float] = None,
        log_interval: int = 50,
        trigger_hook: Optional[
            Callable[[Dict[str, Any]], Optional[TriggerResult]]
        ] = None,
    ):
        ...
        # AMP policy
        self.amp_enabled, self.amp_dtype, use_scaler = get_amp_policy(
            self.device, use_amp
        )
        self.scaler = torch.cuda.amp.GradScaler(
            enabled=(use_scaler and self.amp_enabled)
        )
    ...
    def train_one_epoch(
        self, loader, criterion, *, steps: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        if hasattr(criterion, "reduction"):
            ...
            try:
                ...
                supports_per_sample = criterion.reduction == "none"
            except Exception:
                supports_per_sample = False
            finally:
                try:
        ...
        for batch in loader:
            ...
            if isinstance(batch, (list, tuple)) and len(batch) == 2:
                ...
                # Fallback: treat batch as inputs without targets (self-supervised)
            ...
            with autocast_ctx(self.device, self.amp_enabled, self.amp_dtype):
                ...
                if targets is not None and criterion is not None:
                    if supports_per_sample and self.trigger_hook is not None:
                        ...
                        has_reduction = hasattr(criterion, "reduction")
                        reduction_active = (
                            has_reduction and criterion.reduction != "none"
                        )
                        if reduction_active:
                            reduction_to_restore = criterion.reduction
                            criterion.reduction = "none"
                        try:
                        ...
                        if trig_result is not None:
                            if trig_result.extra_inputs is not None:
                                ...
                                extra_x = to_device(
                                    trig_result.extra_inputs,
                                    self.device,
                                    non_blocking=True,
                                )
                                extra_y = (
                                    to_device(
                                        trig_result.extra_targets,
                                        self.device,
                                        non_blocking=True,
                                    )
                                ...
                                if extra_y is None:
                                    raise ValueError(
                                        "Trigger extra inputs require matching targets."
                                    )
                        ...
                        if weights is not None:
                            ...
                            if w.ndim != 1 or w.shape[0] != loss_vec.shape[0]:
                                raise ValueError(
                                    "Trigger weights must be 1D and match batch length."
                                )
                            weight_sum = w.sum()
                            ...
                            if weight_sum_detached.item() <= 0:
                                raise ValueError(
                                    "Trigger weights must sum to a positive value."
                                )
                    ...
                    if batch_size is None:
                        ...
                        raise ValueError(
                            "Loss function required for supervised training steps."
                        )
                raw_loss = loss
            ...
            loss_detached = raw_loss.detach().to(
                device=total_loss.device, dtype=total_loss.dtype
            )
            ...
            if (step_idx % self.log_interval) == 0:
                ...
                avg_loss = (
                    (total_loss / total_weight).item() if weight_value > 0 else 0.0
                )
                print(
                    f"[Step {step_idx}] loss~{avg_loss:.4f} | "
                    f"thr={m['samples_per_sec']:.1f}/s p50={m['p50_s'] * 1e3:.1f}ms "
                    f"p95={m['p95_s'] * 1e3:.1f}ms",
                    flush=True,
                )

        metrics: Dict[str, Any] = meter.summary()
        if self.device == "cuda":
            try:
