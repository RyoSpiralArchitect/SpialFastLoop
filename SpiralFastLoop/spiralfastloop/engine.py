    ...
    if isinstance(base, Mapping):
        ...
        try:
            if hasattr(base, "default_factory"):
                # Preserve defaultdict default factory semantics
                new_mapping = type(base)(base.default_factory)
            else:
                new_mapping = type(base)()
        except TypeError:
            new_mapping = {}
        for key in base.keys():
            new_mapping[key] = _concatenate_batches(base[key], extra[key])
        return new_mapping
...
def _coerce_weights(
    ...
    *,
    ...
    """Return a detached, 1D weight tensor that matches ``batch_size``."""

    if isinstance(weights, torch.Tensor):
        w = weights.detach()
    else:
        try:
            w = torch.as_tensor(weights).detach()
        except Exception as exc:  # pragma: no cover - defensive path
            raise TypeError("Trigger weights must be convertible to a tensor.") from exc

    if w.ndim == 0:
        # Broadcast a scalar to the whole batch
        w = w.expand(batch_size)
    if w.ndim != 1:
        raise ValueError("Trigger weights must be 1D after broadcasting.")
    if w.shape[0] != batch_size:
        raise ValueError("Trigger weights must match the concatenated batch size.")

    w = w.to(device=device, dtype=dtype)

    if torch.any(w < 0):
        raise ValueError("Trigger weights must be non-negative.")
    if not torch.all(torch.isfinite(w)):
        raise ValueError("Trigger weights must be finite numbers.")

    weight_sum = w.sum()
    if not torch.isfinite(weight_sum):
        raise ValueError("Trigger weights must sum to a finite value.")
                ...
                if targets is not None and criterion is not None:
                    if supports_per_sample and self.trigger_hook is not None:
                        ...
                        trigger_ctx = {
                            ...
                            "outputs": outputs,
                            "loss_vec": loss_vec,
                            "loss_vec_detached": loss_vec.detach(),
                            "device": self.device,
                            "step": step_idx,
                        }
                        trig_payload = self.trigger_hook(trigger_ctx)
                        if isinstance(trig_payload, Mapping) and not isinstance(trig_payload, TriggerResult):
                            trig_result = TriggerResult(**trig_payload)
                        else:
                            trig_result = trig_payload
                        weights = None
                        if trig_result is not None:
                            if not isinstance(trig_result, TriggerResult):
                                raise TypeError("trigger_hook must return TriggerResult, mapping, or None.")
                        ...
                        if weights is not None:
                            ...
                            loss_weight_tensor = weight_sum.detach().to(device=total_loss.device, dtype=total_loss.dtype)
                        else:
                            loss = loss_vec.mean()
