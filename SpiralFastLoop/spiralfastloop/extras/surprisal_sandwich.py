...
class AntiTopKMiddle(LogitsProcessor):
    ...
    def __init__(
        self, start_frac: float = 0.45, end_frac: float = 0.7, topk: int = 5, alpha: float = 10.0
    ):
...
class CoherenceTailBoost(LogitsProcessor):
    ...
    def __init__(
        ...
        if (
            self.tiny is not None
            and self.tiny_tokenizer is not None
            and primary_tokenizer is not None
        ):
            try:
