"""Train a ResNet (or synthetic fallback) with SpiralFastLoop."""
...
def main():
    ...
    try:
        ...
        model = nn.Sequential(
            nn.Linear(128, 256), nn.ReLU(), nn.Linear(256, num_classes)
        )
    ...
    trainer = FastTrainer(
        model, opt, grad_accum=2, log_interval=20, compile_mode="reduce-overhead"
    )
